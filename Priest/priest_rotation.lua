...
if not cache then cache = true
    ...
    function MeleeRange(unit)
        return IsSpellInRange("Smite(Rank 2)", unit) == 1
    end
    ...
    function AnalizeTotem(totem, unit)
        ...
        local melee = MeleeRange(unit)

        if totem == Totems.TREMOR and melee then
            TargetUnit(unit)
    ...
    function AnalizeAuras(unit)
        if (HasAura(Auras.VANISH, unit)
                ...
                and not Cast(Spells.SWD, unit) then
            SpellStopCasting()
        else
            return false
        end
    ...
    print("[PriestScripts] Created by Romain. /reload to disable the frame.")
